...
class AnalyticsEngine:
    ...
    def start(self):
        ...
        while not self._stop.is_set():
            # Procesar feedback manual (si existe) antes de consumir nuevas ventanas
            try:
                self._process_feedback()
            except Exception:
                logger.exception("Error procesando feedback antes del ciclo principal.")
    ...
    # ------------------------------------------------------------------
    # Procesamiento de retroalimentacion manual
    # ------------------------------------------------------------------

    def _process_feedback(self):
        """Consume entradas de retroalimentación publicadas por el API
        en la lista Redis `moodle_feedback` y actualiza `AdaptiveFusion`.
        Formato esperado del elemento: {"timestamp": int, "label": bool, "votes": {model:0/1}}
        """
        ...
        try:
            while True:
                raw = self.redis_client.lpop("moodle_feedback")
                if not raw:
                    break
                try:
                    fb = json.loads(raw)
                except Exception:
                    logger.warning("Feedback malformado: %s", raw)
                    continue

                label = bool(fb.get("label", False))
                votes = fb.get("votes", {})
                for model_key, vote in votes.items():
                    # vote: 1 -> predicted anomaly, 0 -> predicted normal
                    was_correct = (bool(vote) == label)
                    try:
                        self.adaptive_fusion.update(model_key, was_correct)
                    except Exception:
                        logger.exception("Error actualizando peso de fusion para %s", model_key)

                logger.info("Feedback procesado (ts=%s, label=%s)", fb.get("timestamp"), label)
        except Exception as exc:
            logger.error("Error procesando feedback: %s", exc)
