...
| `python scripts/analyze_dataset.py --input mi_dataset.csv` | Analiza un dataset proporcionado (CSV) sin conectarse a Redis/Moodle en vivo; publica en un bucket InfluxDB separado para el dashboard de análisis por lote |
...
Accede en **http://localhost:3100** (admin / admin). Hay **dos dashboards**, cada uno
leyendo de un bucket InfluxDB distinto para no mezclar operación en vivo con análisis
puntuales:

### 1. "Moodle Anomaly Detection - UCI" (tiempo real)
Alimentado por `main.py` (o por `test_simulation.py`/simulaciones con `--influx`) en el
bucket `moodle_metrics`. Incluye:
...
- Estado operativo del broker Redis (longitud de streams, backlog del consumer group, memoria)

### 2. "Moodle Batch Dataset Analysis - UCI" (análisis bajo demanda)
Alimentado por `scripts/analyze_dataset.py --input <archivo.csv>`, en el bucket separado
`moodle_batch_analysis` (config `influxdb.batch_bucket`). Mismos paneles de tráfico,
scores y alertas que el dashboard en vivo (sin el bloque de Redis, que no aplica a un
análisis offline). El rango de tiempo por defecto es amplio (`now-5y`) porque el script
conserva las marcas de tiempo reales del dataset analizado, en vez de remapearlas a
"ahora" — ajusta el selector de tiempo al rango que imprime el script al terminar.
Si el CSV incluye una columna `true_anomaly`, además se calculan precision/recall/F1
en consola y en el CSV de resultados (`logs/analysis_<nombre>.csv`).
...
├── dashboard/
│   ├── influx_writer.py                # RF8: Escritura InfluxDB (bucket configurable)
│   ├── dashboards/
│   │   ├── moodle_anomaly.json         # Dashboard Grafana: tiempo real (bucket moodle_metrics)
│   │   └── moodle_batch_analysis.json  # Dashboard Grafana: análisis por lote (bucket moodle_batch_analysis)
│   └── grafana_provisioning/           # Provisionamiento automático Grafana
│       ├── datasources/influxdb.yaml
│       ├── datasources/redis.yaml
│       └── dashboards/dashboard.yaml
├── scripts/
│   ├── check_connections.py            # Verificación de todos los servicios
│   ├── setup_influxdb.py               # Setup inicial InfluxDB (crea ambos buckets)
│   ├── run_system.py                   # Lanzador con opciones por subsistema
│   └── analyze_dataset.py              # Análisis offline de un dataset (CSV) proporcionado
├── utils/
│   └── config_loader.py
